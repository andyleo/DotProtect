...
namespace DotProtect.Core.Services {
	...
	public class RandomGenerator {
		...
		internal static byte[] Seed(string seed) {
			...
			if (!string.IsNullOrEmpty(seed))
				ret = CoreUtils.SHA256(Encoding.UTF8.GetBytes(seed));
			else
				ret = CoreUtils.SHA256(Guid.NewGuid().ToByteArray());

			for (int i = 0; i < 32; i++) {
				ret[i] *= primes[i % primes.Length];
				ret = CoreUtils.SHA256(ret);
			}
			return ret;
		}
	...
	internal class RandomService : IRandomService {
		...
		public RandomGenerator GetRandomGenerator(string id) {
			...
			byte[] newSeed = seed;
			byte[] idHash = CoreUtils.SHA256(Encoding.UTF8.GetBytes(id));
			for (int i = 0; i < 32; i++)
				newSeed[i] ^= idHash[i];
			return new RandomGenerator(CoreUtils.SHA256(newSeed));
		}
	}
