...
using DotProtect.Renamer.References;
using dnlib.DotNet;
...
namespace DotProtect.Renamer.Analyzers {
	internal class LdtokenEnumAnalyzer : IRenamer {
		...
		void HandleEnum(DotProtectContext context, INameService service, MethodDef method, int index) {
		...
		bool HandleTypeOf(DotProtectContext context, INameService service, MethodDef method, int index) {
