...
namespace DotProtect.Renamer.Analyzers {
	internal class JsonAnalyzer : IRenamer {
		...
		void Analyze(DotProtectContext context, INameService service, TypeDef type, ProtectionParameters parameters) {
		...
		void Analyze(DotProtectContext context, INameService service, MethodDef method, ProtectionParameters parameters) {
			if (GetJsonContainerAttribute(method.DeclaringType) != null && method.IsConstructor) {
				service.SetParam(method, "renameArgs", "false");
			}
		}

		void Analyze(DotProtectContext context, INameService service, PropertyDef property, ProtectionParameters parameters) {
			if (ShouldExclude(property.DeclaringType, property)) {
				service.SetCanRename(property, false);
			}
		}

		void Analyze(DotProtectContext context, INameService service, FieldDef field, ProtectionParameters parameters) {
