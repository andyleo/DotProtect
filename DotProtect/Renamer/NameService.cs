...
namespace DotProtect.Renamer {
	public interface INameService {
		...
		void MarkHelper(IDnlibDef def, IMarkerService marker, DotProtectComponent parentComp);
	}
	...
	internal class NameService : INameService {
		...
		public NameService(DotProtectContext context) {
        ...
        public string ObfuscateName(string name, RenameMode mode)
        {
            ...
            byte[] hash = CoreUtils.Xor(CoreUtils.SHA1(Encoding.UTF8.GetBytes(name)), nameSeed);
            for (int i = 0; i < 100; i++)
            {
                newName = ObfuscateNameInternal(hash, mode);
                if (!identifiers.Contains(MakeGenericName(newName, count)))
                    break;
                hash = CoreUtils.SHA1(hash);
            }
		...
		public string RandomName(RenameMode mode) {
			return ObfuscateName(CoreUtils.ToHexString(random.NextBytes(16)), mode);
		}
		...
		public void MarkHelper(IDnlibDef def, IMarkerService marker, DotProtectComponent parentComp) {
